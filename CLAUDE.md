...
**Weeks 1–5 are complete; week 6 (ship) has not started.** Ingestion, data accounting, QC, and harmonization are deep; the model fits the full 28-test region set with primary and secondary FDR families corrected separately, a harmonized-vs-unharmonized sensitivity arm, the §2.5.4 completer comparison, and tolerance-bounded slope recovery against injected truth across all 28 tests. The one item still outstanding from earlier weeks is the §1.2 OpenNeuro audit, which decides only whether v1.0 carries a longitudinal *real-data* claim. Stage docstrings state precisely what is stubbed and what remains.
...
Markers declared in `pyproject.toml`: `slow` (statistical recovery over larger fixtures — the model slope suite and the four §2.3.2 harmonization criteria), `optional` (cross-checks needing something the default install does not have: `neuroCombat`, a `[project.optional-dependencies]` extra deliberately kept out of the dev group so it cannot enter the default resolution; and the ABIDE cross-check, which needs `data/abide_pcp` + `data/abide_fs6` and skips on a path check). Coverage is printed but not gated — there is deliberately no `--cov-fail-under`. It is measured in two jobs (`test` and `recovery`), each writing a distinct `COVERAGE_FILE`; the `coverage` job combines them and publishes the badge. `relative_files = true` is what makes the combine correct, and it applies at *measurement* time — data files recorded without it report 0% once it is on, rather than failing.

CI jobs: `lint`, `types`, `test` (`-m "not slow"`), `recovery` (`-m slow`), `xcheck` (`-m optional`, with the extra), `coverage` (combines the two data files, publishes the badge), `e2e`, `nf`, `publish`. A separate `pages` workflow republishes the example report. **ruff is pinned exactly** in both `pyproject.toml` and `.pre-commit-config.yaml`, and the two must stay in lockstep — when they drifted, pre-commit and CI formatted the same file differently and CI failed on a commit that had not touched it.
...
Two adapters exist: `synthetic.py` and `abide_pcp.py`. `adapters/fs6_tables.py` sits beside them but is **not** an adapter and is deliberately absent from `build_adapter`'s registry — it reads pre-aggregated `asegstats2table` output for the cross-check test only, and wiring it in as an adapter would make "we parse FreeSurfer's native output" claimable against a table someone else already aggregated. The table → measurement-row conversion they share lives in `adapters/freesurfer_rows.py`; only *metadata* resolution is dataset-specific, and two copies of the row extraction would drift into datasets disagreeing about what a thickness is.

**Adapters select stats files by exact filename, never by `*.stats` glob.** The parser identifies tables by filename, so `lh.aparc.a2009s.stats` is also an lh aparc table and `lh.entorhinal_exvivo.stats` re-reports a structure the Desikan-Killiany table already reports. A glob emits two rows for one `subject × session × region × measure` key, which `write_canonical` rejects — correctly, since neither row is wrong; they answer different questions. `abide_pcp.CORE_TABLES` is the allowlist (module-level, not a class attribute), and a test asserts the extra tables are ignored when present.
...
The **modeling boundary** is decomposed, not labelled. `ModelExclusion` separates `outside_modeled_region_set` (a scope decision) from `incomplete_model_covariates` (a data limitation that biases the remaining sample), and falls back to `not_modeled_cause_unavailable` when the model's per-region inputs were not supplied. These have opposite implications, so the single catch-all that used to sit here let the funnel reconcile while reporting a cause that was false — which is worse than not reconciling, because it looks like an answer. Passing `model_fits` into `build_accounting` is what makes the split possible.

Now that the model fits all 28 regions, synthetic runs lose *nothing* at this boundary and ABIDE attributes the whole of it to `design_not_identifiable` — cross-sectional data cannot identify a longitudinal specification, so no region is estimable and the covariate incompleteness underneath never gets a chance to bite. That ordering is deliberate: `incomplete` is computed only from what remains after the unidentifiable regions are removed, because reporting a study-design limitation as a data-quality one inverts what a reader should conclude.
...
- **The dfsp-spirit "FreeSurfer 6" tables are FreeSurfer 5.1.** Its aggregated stats tables carry values bit-identical to ABIDE PCP's FreeSurfer 5.1 output — all 28,976 shared observations, max absolute difference `0.0` — which two `recon-all` runs at different releases cannot produce. Its own `brainvol` file corroborates it, pairing the FS6 column name `EstimatedTotalIntraCranialVol` with the FS5.1 value. So the planned cross-*version* comparison is impossible; what replaced it is an exact-equality check of morphline's parse → map → canonicalize path against FreeSurfer's own aggregation tools, which is sharper (a hemisphere swap shows as an exact mismatch rather than hiding in version noise). Recorded as BUILD_PLAN revision 7 and pinned by `test_the_tables_are_not_an_independent_freesurfer_6_derivation`. Do not reintroduce rank-correlation tolerances here.
...
- **The sensitivity arm proves it is a comparison before reporting one.** `compare_arms` takes `applicable` from whether the two frames' *values* actually differ, not from `harmonization.enabled` — a run with harmonization on that changes nothing (every batch below `min_batch_size`, which is `config/test.yaml` by design) would otherwise present one fit twice under two headings. Likewise, zero comparable regions gets its own note: "0 of 28 estimated under both arms" must not read as agreement. Both degenerate paths are exercised in CI — `test.yaml` for the first, ABIDE for the second.
- **Recovery targets are measured off `true_biological_value`, never recomputed from the generator's coefficients.** `injected_interaction` gets the target by OLS on the recorded truth column. Re-deriving `base × direction × b_dxtime` in the test would make it agree with the generator by construction — it would keep passing if `EXPANDING_STRUCTURES` were dropped and ventricles were injected as shrinking, because the expectation would be wrong the same way. Same rule as the ComBat γ target above.
- **The recovery fixture pins `multiplicative_effect = 1.0`, and that is load bearing.** `observed = biological · mult_site + add_site + ε`, so a multiplicative site effect *scales the slope*: the injected interaction then differs per site and the pooled estimate is a weighted average with no single right answer to compare against. An additive effect shifts intercepts only and leaves every slope untouched, so the sites still differ and the target stays exact. Multiplicative site effects are covered by the harmonization suite instead.
- **Completer imbalance is reported as a standardized mean difference, never a p-value.** A significance test there answers "is the sample large enough to detect a difference", which is not the question; §2.5.4 needs to know whether the groups differ *enough that MAR is load-bearing*. Subjects with no observations at all are counted in `n_without_baseline` and never enter the comparison — they are the likeliest informative missingness, so folding them in or dropping them silently would bias the check meant to detect bias. A cross-sectional dataset reports `applicable: false` with the reason, because an empty table reads as "checked and found nothing".
...
The `OWNER` placeholder is resolved across all four sites it occupied — [README.md](README.md) badges, [nextflow.config](nextflow.config)'s GHCR image, [nextflow_schema.json](nextflow_schema.json)'s `$id`, and [containers/Dockerfile](containers/Dockerfile)'s `image.source` label. The remote is `github.com/mattglittenberg/morphline` and the image is `ghcr.io/mattglittenberg/morphline`. The repo is named for the package, not for the working directory (`neuroimaging-pipeline`) — CI derives the image from `github.repository`, so the two stay in step only as long as that holds. Search for stragglers with `git grep OWNER`, not a `--include`-filtered grep: the Dockerfile has no extension and was missed exactly that way once.
